...
import {
  ...
  itIT, dateItIT, useMessage, useDialog,
} from 'naive-ui';
import api from '../api/client';
import CommessaForm from '../components/CommessaForm.vue';
import Analitiche from '../components/Analitiche.vue';

// Wrap inner in a separate component so we can use useMessage/useDialog
import { defineComponent } from 'vue';

const Inner = defineComponent({
  components: { CommessaForm, Analitiche },
  setup() {
    ...
    return {
      tab, commesse, loading, utenti, ditte, editingId, editForm, filtro, form, saveError,
      selectedCommessa, commesseFiltrate, isFiltroAttivo, visibleTabs,
      remove, startEdit, cancelEdit, saveEdit, handleCreate,
      getDittaNomi, getReferenteNomi, FORM_VUOTO,
    };
  },
  template: `
    <div class="page">
      ...
      <div style="display: flex; gap: 0.5rem; margin-bottom: 1.5rem; border-bottom: 2px solid var(--border);">
        <button v-for="t in visibleTabs" :key="t" @click="tab = t; if (t === 'creazione') { form = FORM_VUOTO(); saveError = ''; }"
                :style="{
          ...
          <template v-else>{{ t.charAt(0).toUpperCase() + t.slice(1) }}</template>
        </button>
      </div>
